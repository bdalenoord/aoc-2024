...
DROP SCHEMA IF EXISTS day7_part1 CASCADE;
CREATE SCHEMA day7_part1;
CREATE TABLE day7_part1.input (data TEXT);
COPY day7_part1.input FROM '/days/day_7/input.txt';

CREATE VIEW day7_part1.split AS SELECT regexp_split_to_array(REPLACE(data, ':', ''), ' ') FROM day7_part1.input;

CREATE OR REPLACE FUNCTION day7_part1.check(
...
BEGIN
    ...
    IF day7_part1.check(_expected_result, _values, _current_value + _new_number, _position + 1) THEN
        RETURN TRUE;
    ELSEIF day7_part1.check(_expected_result, _values, _current_value * _new_number, _position + 1) THEN
        RETURN TRUE;
    END IF;
...
CREATE OR REPLACE FUNCTION day7_part1.is_valid(_equation TEXT[]) RETURNS BOOLEAN AS $$
DECLARE
    _result BIGINT;
    _inputs BIGINT[];
BEGIN
    ...
    RETURN day7_part1.check(_result, _inputs, _inputs[1], 2);
END $$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION day7_part1.part1() RETURNS BIGINT AS $$
DECLARE
    _equation TEXT[];
    _result BIGINT = 0;
    _valid_equation_count BIGINT = 0;
BEGIN
    FOR _equation IN SELECT * FROM day7_part1.split LOOP
        IF day7_part1.is_valid(_equation) THEN
...
SELECT day7_part1.part1() "The answer for part 1 of day 7 is";